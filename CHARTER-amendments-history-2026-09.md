# CHARTER — amendment log, part 1: 2026-09-18 to 2026-09-21 (Rachel, AI Finance Assistant)

_The **first twenty-two amendments**, moved here **verbatim and by line index, nothing retyped**, on
**2026-09-24**. Owner ruling (Minda) the same day: split it. The live log is `CHARTER-amendments.md`, which keeps
its own header and **Amendments 23 onwards**._

_**The trigger was the write path, and it is worth stating exactly rather than as "the file got big."** Adding
Amendment 28 took `CHARTER-amendments.md` to **44,214 bytes**. The most Rachel has ever successfully written in one
call is **43,856** (2026-09-21), and **Drive has no append** — `update_file` changes only title and parent, so a
file is created by emitting its whole content in a single call. The amendment log had therefore passed the point
where it could be written at all, and Amendment 28 existed only in git until this split. **That is a different
failure from the one the issue-log splits answered** (a live table too large to edit) and different again from the
31,316-byte truncation ceiling that no longer exists — three distinct limits, one scheme. `RA-31` carries all of
them._

_**Why the cut fell here.** Amendments 1–22 are a run of dated italic paragraphs; 23 onwards are headed sections.
The boundary is the format change, which is also 2026-09-22 — the day the log started recording each amendment as
its own section. So the split follows a seam that was already in the file rather than one imposed on it, and no
numbered amendment was moved. **Nothing was reworded, renumbered or tidied on the way across.**_

_**This file is governed exactly as `CHARTER.md` is** — `Raw/`-only for cross-KB amendments, archive-then-recreate
with byte verification, and mirrored to git. Splitting a file changes nothing about who may write it or how._

---

## Amendments 1–22, oldest first
